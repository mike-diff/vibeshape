import type { Importance, Shape, ShapeNode } from './types.js';
...
export function slugify(text: string): string {
  const slug = text
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
  if (!slug) throw new Error(`cannot derive a slug from "${text}"`);
  return slug;
}

export function findNode(shape: Shape, id: string): ShapeNode | undefined {
...
function findInSubtree(node: ShapeNode, id: string): ShapeNode | undefined {
  if (node.id === id) return node;
  if (!id.startsWith(`${node.id}/`)) return undefined;
...
export function findParent(shape: Shape, id: string): ShapeNode | undefined {
  const lastSlash = id.lastIndexOf('/');
  if (lastSlash === -1) return undefined;
  return findNode(shape, id.slice(0, lastSlash));
}

export function areaOf(shape: Shape, id: string): ShapeNode | undefined {
 ...
 */
export function addNode(shape: Shape, parentId: string, options: AddOptions): ShapeNode {
  const slug = options.slug ?? slugify(options.title);
  const node: ShapeNode = { id: slug, title: options.title };
  if (options.intent) node.intent = options.intent;
  if (options.importance) node.importance = options.importance;

  if (parentId === '/') {
    if (shape.areas.some((a) => a.id === slug)) throw new Error(`area "${slug}" already exists`);
...
export function removeNode(shape: Shape, id: string): ShapeNode {
  if (!id.includes('/')) {
  ...
  if (!parent || index === -1) throw new Error(`node "${id}" not found`);
...
export function moveNode(shape: Shape, id: string, newParentId: string): ShapeNode {
...
function reId(node: ShapeNode, newId: string): void {
  const oldId = node.id;
  node.id = newId;
  for (const child of node.children ?? []) {
...
export function walk(node: ShapeNode, visit: (node: ShapeNode, depth: number) => void, depth = 0): void {
  visit(node, depth);
