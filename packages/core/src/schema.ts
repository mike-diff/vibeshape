...
function checkString(
  ...
  path: string,
  value: unknown,
  options: { optional?: boolean; pattern?: RegExp; patternHint?: string } = {},
): void {
  if (value === undefined) {
    if (!options.optional) errors.push(`${path}: required`);
    return;
  }
...
function checkEnum(errors: string[], path: string, value: unknown, allowed: readonly string[]): void {
...
export function nodeErrors(raw: unknown, path = 'node'): string[] {
  const errors: string[] = [];
  ...
  if (raw.evidence !== undefined) {
    if (!Array.isArray(raw.evidence)) {
      ...
      raw.evidence.forEach((entry, i) => {
        ...
        if (typeof entry.type !== 'string' || !(EVIDENCE_TYPES as readonly string[]).includes(entry.type)) {
...
export function manifestErrors(raw: unknown): string[] {
  ...
  if (raw.schemaVersion !== 1) errors.push('manifest.schemaVersion: must be 1');
...
export function parseNode(raw: unknown): ShapeNode {
  const errors = nodeErrors(raw);
  if (errors.length > 0) throw new Error(errors.join('; '));
  return raw as ShapeNode;
}
